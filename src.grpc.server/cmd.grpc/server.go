...
//	pb "gitlab.iblog.pro/cobra/libvirt/internal/protoc"
...
//        logs "gitlab.iblog.pro/cobra/libvirt/internal/cobra/logs"
        ...
        Virsh "gitlab.iblog.pro/cobra/libvirt/internal/cobra/core/virshgrpc"
...
func (s *server) SayHello(ctx context.Context, in *pb.HelloRequest) (*pb.HelloReply, error) {
        fmt.Printf("Received: %v %v", in.GetName(), in.GetUuid())
        return &pb.HelloReply{Message: "Hello " + in.GetName(), Code: 200, Uuid: in.GetUuid()}, nil
}
*/
...
func main() {
		...
		if err != nil {
			fmt.Printf("Error Starting HTTP server - localhost:6060\n")
			os.Exit(1)
		}
        }()
	...
	grpcPanicRecoveryHandler := func(p any) (err error) {
		panicsTotal.Inc()
		_ = level.Error(rpcLogger).Log("msg", "recovered from panic", "panic", p, "stack", debug.Stack())
	...
	grpcSrv := grpc.NewServer(
        ...
        pb.RegisterVirshServer(grpcSrv, Virsh.NewServer())
//        pb.RegisterGreeterServer( grpcSrv, &server{})
//	t := &pb.TestPingService{}
//	pb.RegisterTestServiceServer(grpcSrv, t)
	...
	g.Add(func() error {
		...
		_ = level.Info(logger).Log("msg", "starting gRPC server", "addr", l.Addr().String())
	...
	g.Add(func() error {
		...
		httpSrv.Handler = m
		_ = level.Info(logger).Log("msg", "starting HTTP server", "addr", httpSrv.Addr)
		return httpSrv.ListenAndServe()
	}, func(error) {
		if err := httpSrv.Close(); err != nil {
			_ = level.Error(logger).Log("msg", "failed to stop web server", "err", err)
		}
	})
