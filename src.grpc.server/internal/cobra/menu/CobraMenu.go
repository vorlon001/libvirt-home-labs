...
func initVM(increment int) {
	...
	var core *Model.Core = store.Singleton[Model.Core]()
	...
	for stop==0 {
		...
		stopCodes, err := virt.GetDomainState(c.VMNAME)
		if err != nil {
  ...
  var subattachInerfaceVM = &cobra.Command{
	...
	if err != nil {
        ...
        for _,j := range doms {
			...
			err = interfaceVM.CreateMacAddress(c.Network.MagicMac)
			if err != nil {
				logs.Log.WithFields(logrus.Fields{ "err": err, }).Info("Cobra Event Error")
				return
			}
