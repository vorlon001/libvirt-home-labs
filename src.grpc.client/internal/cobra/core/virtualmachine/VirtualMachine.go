...
type VirtualMachineInterface interface {
        ...
        GetLookup(name string) (libvirt.Domain, error)
        GetDomains() ([]libvirt.Domain, error)
        GetDomainState(dom string) (libvirt.DomainState, error)
...
func (ret *VirtualMachine) GetLookup(name string) (libvirt.Domain, error) {
        return ret.GetLibvirt().DomainLookupByName(name)
}


// Domains returns a list of all domains managed by libvirt.
func (ret *VirtualMachine) GetDomains() ([]libvirt.Domain, error) {
        // these are the flags as passed by `virsh list --all`
        flags := libvirt.ConnectListDomainsActive | libvirt.ConnectListDomainsInactive
        domains, _, err := ret.GetLibvirt().ConnectListAllDomains(1, flags)
        return domains, err
}


// DomainState returns state of the domain managed by libvirt.
func (ret *VirtualMachine)  GetDomainState(dom string) (libvirt.DomainState, error) {
        d, err := ret.GetLookup(dom)
        if err != nil {
                return libvirt.DomainNostate, err
        }

        state, _, err := ret.GetLibvirt().DomainGetState(d, 0)
	...
	virt,err := Virtinit()
        ...
        for _,j := range doms {
                if j.Name == core.VMid {
                        ...
                        err = xml.Unmarshal([]byte(domainGetXMLDesc), &t)
                        if err != nil {
                                logs.Log.WithFields(logrus.Fields{ "err": err, }).Info("Cobra Event Error")
                                return "Error", err
                        }
...
//        var ret VirtualMachine //VirtualMachineState
	...
	logs.Log.WithFields(logrus.Fields{ "state": state, "maxmem": maxmem, "mem": mem, "ncpu": ncpu, "cputime": cputime, "err": err, }).Info("Inside VirtualMachineState Run")
        ...
        rState, _, err := ret.Libvirt.DomainGetState(d, 0)
        logs.Log.WithFields(logrus.Fields{ "Message": rState, "err": err, "id": id, }).Info("Inside VirtualMachineState Point: 00001")
        if err != nil {
                PanicRecover.Herr(err)
                return "Error", err
        }
